import { describe, it, expect } from "bun:test"
import { parseGithubUrl } from "./url"

describe("parseGithubUrl", () => {
  it("parses standard HTTPS URL", async () => {
    const result = await parseGithubUrl("https://github.com/owner/repo")
    expect(result).toEqual({ owner: "owner", repo: "repo" })
  })

  it("parses HTTPS URL with .git suffix", async () => {
    const result = await parseGithubUrl("https://github.com/owner/repo.git")
    expect(result).toEqual({ owner: "owner", repo: "repo" })
  })

  it("parses SSH git URL", async () => {
    const result = await parseGithubUrl("git@github.com:owner/repo")
    expect(result).toEqual({ owner: "owner", repo: "repo" })
  })

  it("parses SSH git URL with .git suffix", async () => {
    const result = await parseGithubUrl("git@github.com:owner/repo.git")
    expect(result).toEqual({ owner: "owner", repo: "repo" })
  })

  it("parses org URL with dash", async () => {
    const result = await parseGithubUrl("https://github.com/my-org/my-repo")
    expect(result).toEqual({ owner: "my-org", repo: "my-repo" })
  })

  it("parses repo name with dashes", async () => {
    const result = await parseGithubUrl(
      "https://github.com/owner/my-awesome-repo",
    )
    expect(result).toEqual({ owner: "owner", repo: "my-awesome-repo" })
  })

  it("throws on non-GitHub URL", async () => {
    expect(parseGithubUrl("https://gitlab.com/owner/repo")).rejects.toThrow()
  })

  it("throws on invalid URL format", async () => {
    expect(parseGithubUrl("not-a-url")).rejects.toThrow()
  })

  it("throws on empty string", async () => {
    expect(parseGithubUrl("")).rejects.toThrow()
  })

  it("throws on URL with only owner", async () => {
    expect(parseGithubUrl("https://github.com/owner")).rejects.toThrow()
  })
})
